// The stat indexer is a stateless daemon (no server): it polls the
// index_queue table in postgres for files needing stat indexing, fetches
// metadata from S3, and records the results. Files enter the queue by
// self-seeding from the files table, which the crawler (and later the API)
// populates. It has no in-process concurrency; scale out by running more
// replicas.
package main
...
func main() {
	...
	indexType := "stat"
	queue := indexer.NewPGQueue(pool, indexType, indexer.StoreStatResult)
	...
	if err := indexer.Run(ctx, indexType, indexerCfg, queue, stat.Process); err != nil {
		slog.Error("stat indexer failed", "error", err)
		os.Exit(1)
	}
}
