...
func main() {
	...
	s3, err := storage.NewWithPublicEndpoint(cfg.S3.Endpoint, cfg.S3.PublicEndpoint, cfg.S3.AccessKeyID, cfg.S3.SecretAccessKey, false, cfg.S3.Bucket)
	if err != nil {
		slog.Error("storage init failed", "error", err)
		os.Exit(1)
	}

	srv := files.New(cfg.GrpcAddr, s3, db.New(pool))
