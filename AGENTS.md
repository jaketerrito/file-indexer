...
- Playwright e2e tests live in `web/e2e/` and run against the tilt-deployed
  app via the shared gateway: `just test-e2e` (requires `just up`; the
  crawler indexes the seed dataset when the session starts). They also run
  as the `test-e2e` Tilt resource under `just ci` — the crawler is
  auto-trigger there because `tilt ci` never runs manual resources. Keep
  `web/e2e` excluded from vitest (`web/vitest.config.ts`).
